...
exports.register = async (req, res, next) => {
    try {//pass data
        ...
        const { first_name,user_contact_number,user_email,user_password } = req.body;
        //const duplicate = await UserServices.getUserByEmail(email);
       // if (duplicate) {
          //  throw new Error(`UserName ${email,contact_number}, Already Registered`)
        //}
        const successRes = await UserServices.registerUser(first_name,user_contact_number,user_email,user_password );
...
exports.login = async (req, res, next) => {
    try {
       ...
       // if (!email || !password) {
        ...
        // Creating Token
        let tokenData;
        tokenData = { user_id: user._id, user_email: user.user_email,name1:user.first_name,Phn:user.user_contact_number };
